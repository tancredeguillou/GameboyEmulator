...
// ----------------------------------------------------------------------
static uint8_t look_for_interrupt(cpu_t *cpu)
{
    uint8_t interrupt = cpu->IF & cpu->IE;
    for (interrupt_t i = 0; i < INTERRUPT_BOUND; i++)
    {
        if (((interrupt >> i) & 1) == 1)
        {
...
{
    M_REQUIRE_NON_NULL(cpu);
    uint8_t interruption = look_for_interrupt(cpu);
    // if there is an interruption to be handled.
    if (1 == cpu->IME && interruption != NO_INTERRUPT)
    {
        cpu->IME = 0;
        cpu->IF &= ~interruption;
        cpu_SP_push(cpu, cpu->PC);
        cpu->PC = interrupt_address(interruption);
...
int cpu_cycle(cpu_t *cpu)
{
    ...
    if (1 == cpu->HALT)
    {
        ...
        if (NO_INTERRUPT != look_for_interrupt(cpu))
        {
            cpu->HALT = 0;
